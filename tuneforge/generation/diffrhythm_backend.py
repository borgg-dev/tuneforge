...
class DiffRhythmBackend:
    ...
    def load(self) -> None:
        ...
        try:
            ...
            # Load lyrics generator for vocal requests
            from tuneforge.generation.lyrics_generator import LyricsGenerator
            self._lyrics_gen = LyricsGenerator(device=self.device)
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        # Handle lyrics/vocals using the lyrics generator
        from tuneforge.generation.lyrics_generator import extract_genre, extract_mood

        detected_genre = extract_genre(prompt)
        detected_mood = extract_mood(prompt)
        ...
        if lyrics and lyrics not in ("[Instrumental]", "[Vocals]"):
            ...
            # Vocal requested but no lyrics — generate from prompt using GPT-2
            logger.info("Generating lyrics from prompt (vocals requested, no lyrics provided)")
            if self._lyrics_gen:
                generated = self._lyrics_gen.generate(
                    prompt=prompt,
                    genre=detected_genre,
                    mood=detected_mood,
                    duration_seconds=duration_seconds,
                )
                lyrics = self._ensure_lrc_format(generated, duration_seconds)
                logger.info(f"Generated {len(generated.splitlines())} lines of lyrics")
            else:
                lyrics = "[Instrumental]"
                logger.warning("Lyrics generator not available, falling back to instrumental")
        else:
            lyrics = "[Instrumental]"
